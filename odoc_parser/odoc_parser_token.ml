...
type code_block_tag =
  ...
  | `Binding of string Odoc_parser_loc.with_location * string Odoc_parser_loc.with_location ]
...
(* [`Minus] and [`Plus] are interpreted as if they start list items. Therefore,
   for error messages based on [Odoc_parser_token.describe] to be accurate, formatted
