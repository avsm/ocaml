...
module Warning = Odoc_parser_warning

type t = {
  ast : Odoc_parser_ast.t;
  warnings : Odoc_parser_warning.t list;
...
(* odoc uses an ocamllex lexer. The "engine" for such lexers is the standard
   ...
   to convert from byte offset to line number / column number - a Odoc_parser_loc.point,
   and additionally for converting back from a Odoc_parser_loc.point to a Lexing.position.
*)
  ...
  let rec scan_to_last_newline reversed_newlines_prefix =
    ...
    | (line_in_comment, line_start_offset) :: prefix ->
        if line_start_offset > byte_offset then scan_to_last_newline prefix
          ...
          in
          { Odoc_parser_loc.line = line_in_file; column = column_in_file }
  in
  scan_to_last_newline reversed_newlines

(* Given a Odoc_parser_loc.point and the result of [parse_comment], this function returns
   a valid Lexing.position *)
let position_of_point : t -> Odoc_parser_loc.point -> Lexing.position =
 fun v point ->
  let { reversed_newlines; original_pos; _ } = v in
  let line_in_comment = point.Odoc_parser_loc.line - original_pos.pos_lnum + 1 in
  ...
  let pos_lnum = point.Odoc_parser_loc.line in
...
let parse_comment ~location ~text =
  ...
  let token_stream =
    ...
    in
    let input : Odoc_parser_lexer.input =
      {
    ...
    in
    Odoc_parser_stream.from (fun _token_index -> Some (Odoc_parser_lexer.token input lexbuf))
  in
  let ast, warnings = Odoc_parser_syntax.parse warnings token_stream in
...
let deindent : what:string -> loc:Odoc_parser_loc.span -> string -> string * Odoc_parser_warning.t list
    =
 fun ~what ~loc s ->
  ...
  let warning =
    if least_amount_of_whitespace < offset then
      [ Odoc_parser_parse_error.not_enough_indentation_in_code_block ~what loc ]
    else []
  in
  let drop n line =
...
let code_block_content ~what ~loc s =
  let indent = loc.Odoc_parser_loc.start.column in
