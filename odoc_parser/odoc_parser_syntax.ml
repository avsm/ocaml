(* This module is a recursive descent parser for the ocamldoc syntax. The parser
   consumes a token stream of type [Odoc_parser_token.t Odoc_parser_stream.t], provided by the lexer,
...
type input = {
  tokens : Odoc_parser_token.t Odoc_parser_loc.with_location Odoc_parser_stream.t;
...
let junk input = Odoc_parser_stream.junk input.tokens

let peek input =
  match Odoc_parser_stream.peek input.tokens with
  | Some token -> token
  | None -> assert false
...
module Table = struct
  module Light_syntax = struct
    let valid_align = function
      | [ { Odoc_parser_loc.value = `Word w; _ } ] -> (
    ...
    let create ~grid ~align : Odoc_parser_ast.table =
      let cell_to_block (x, k) =
        let whole_loc = Odoc_parser_loc.span (List.map (fun x -> x.Odoc_parser_loc.location) x) in
        match x with
        | [] -> ([], k)
        | _ -> ([ Odoc_parser_loc.at whole_loc (`Paragraph x) ], k)
      in
      let row_to_block = List.map cell_to_block in
      let grid_to_block = List.map row_to_block in
      ((grid_to_block grid, align), `Light)

    let with_kind kind : 'a with_location list list -> 'a Odoc_parser_ast.row =
      List.map (fun c -> (c, kind))

    let from_raw_data grid : Odoc_parser_ast.table =
      match grid with
  ...
  module Heavy_syntax = struct
    let create ~grid : Odoc_parser_ast.table = ((grid, None), `Heavy)
    let from_grid grid : Odoc_parser_ast.table = create ~grid
  end
end
...
module Reader = struct
  let until_rbrace_or_eof input acc =
    let rec consume () =
      ...
      | `End ->
          Odoc_parser_parse_error.end_not_allowed next_token.location ~in_what:"table"
          |> add_warning input;
          junk input;
          `End (acc, next_token.location)
...
let npeek n input = Odoc_parser_stream.npeek n input.tokens
...
(* Check that the token constructors above actually are all in [Odoc_parser_token.t]. *)
let _check_subset : token_that_always_begins_an_inline_element -> Odoc_parser_token.t =
 fun t -> (t :> Odoc_parser_token.t)
...
let rec inline_element :
    input -> Odoc_parser_loc.span -> _ -> Odoc_parser_ast.inline_element with_location =
 fun input location next_token ->
  match next_token with
  | `Space _ as token ->
      junk input;
      Odoc_parser_loc.at location token
  | `Word _ as token ->
      junk input;
      Odoc_parser_loc.at location token
  ...
  | `Minus ->
      junk input;
      Odoc_parser_loc.at location (`Word "-")
  | `Plus ->
      junk input;
      Odoc_parser_loc.at location (`Word "+")
  | `Bar ->
  ...
  | `Begin_style s as parent_markup ->
      ...
      let location = Odoc_parser_loc.span [ location; brace_location ] in

      if content = [] then
        Odoc_parser_parse_error.should_not_be_empty
          ~what:(Odoc_parser_token.describe parent_markup)
          location
        |> add_warning input;

      Odoc_parser_loc.at location (`Styled (s, content))
  | `Simple_reference r ->
      junk input;

      let r_location = Odoc_parser_loc.nudge_start (String.length "{!") location in
      let r = Odoc_parser_loc.at r_location r in

      Odoc_parser_loc.at location (`Reference (`Simple, r, []))
  | `Begin_reference_with_replacement_text r as parent_markup ->
      ...
      Odoc_parser_loc.at location (`Reference (`With_text, r, content))
  | `Simple_link u ->
      junk input;
      ...
      if u = "" then
        Odoc_parser_parse_error.should_not_be_empty
          ~what:(Odoc_parser_token.describe next_token)
          location
        |> add_warning input;

      Odoc_parser_loc.at location (`Link (u, []))
  | `Begin_link_with_replacement_text u as parent_markup ->
      ...
      `Link (u, content) |> Odoc_parser_loc.at (Odoc_parser_loc.span [ location; brace_location ])
...
and delimited_inline_element_list :
    parent_markup:[< Odoc_parser_token.t ] ->
    parent_markup_location:Odoc_parser_loc.span ->
    requires_leading_whitespace:bool ->
    input ->
    Odoc_parser_ast.inline_element with_location list * Odoc_parser_loc.span =
 fun ~parent_markup ~parent_markup_location ~requires_leading_whitespace
  ...
  let rec consume_elements :
      ...
      Odoc_parser_ast.inline_element with_location list ->
      Odoc_parser_ast.inline_element with_location list * Odoc_parser_loc.span =
   fun ~at_start_of_line acc ->
    ...
    | `Single_newline ws ->
        junk input;
        let element = Odoc_parser_loc.same next_token (`Space ws) in
        consume_elements ~at_start_of_line:true (element :: acc)
    | `Blank_line ws as blank ->
        Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe blank)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          next_token.location
        |> add_warning input;
    ...
    | (`Minus | `Plus) as bullet ->
        (if at_start_of_line then
           let suggestion =
             Printf.sprintf "move %s so it isn't the first thing on the line."
               (Odoc_parser_token.print bullet)
           in
           Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe bullet)
             ~in_what:(Odoc_parser_token.describe parent_markup)
        ...
        consume_elements ~at_start_of_line:false acc
    | other_token ->
        Odoc_parser_parse_error.not_allowed
          ~what:(Odoc_parser_token.describe other_token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          next_token.location
        |> add_warning input;
  ...
  | _ ->
      if requires_leading_whitespace then
        Odoc_parser_parse_error.should_be_followed_by_whitespace
          ~what:(Odoc_parser_token.print parent_markup)
          parent_markup_location
        |> add_warning input;
      consume_elements ~at_start_of_line:false []
...
(* Consumes tokens that make up a paragraph.
   ...
   and so on. *)
let paragraph : input -> Odoc_parser_ast.nestable_block_element with_location =
 fun input ->
  ...
  let rec additional_lines :
   ...
   fun acc ->
    ...
    | { value = `Single_newline ws; location }
      ...
      :: _ ->
        junk input;
        let acc = Odoc_parser_loc.at location (`Space ws) :: acc in
  ...
  `Paragraph elements |> Odoc_parser_loc.at (Odoc_parser_loc.span (List.map Odoc_parser_loc.location elements))
...
(* The block parsing loop, function [block_element_list], stops when it
   ...
   stopped it, with a type more precise than [Odoc_parser_token.t stream_head]: if it was
...
type stopped_implicitly =
  ...
  | `Minus
  | `Plus
  | Odoc_parser_token.section_heading
  | Odoc_parser_token.media_markup
  | Odoc_parser_token.tag ]

(* Ensure that the above two types are really subsets of [Odoc_parser_token.t]. *)
let _check_subset : stops_at_delimiters -> Odoc_parser_token.t = fun t -> (t :> Odoc_parser_token.t)
let _check_subset : stopped_implicitly -> Odoc_parser_token.t = fun t -> (t :> Odoc_parser_token.t)
...
type ('block, 'stops_at_which_tokens) context =
  | Top_level : (Odoc_parser_ast.block_element, stops_at_delimiters) context
  | In_implicitly_ended :
      [ `Tag | `Shorthand_list ]
      -> (Odoc_parser_ast.nestable_block_element, stopped_implicitly) context
  | In_explicit_list : (Odoc_parser_ast.nestable_block_element, stops_at_delimiters) context
  | In_table_cell : (Odoc_parser_ast.nestable_block_element, stops_at_delimiters) context
  | In_code_results : (Odoc_parser_ast.nestable_block_element, code_stop) context
...
let accepted_in_all_contexts : type block stops_at_which_tokens.
    ...
    Odoc_parser_ast.nestable_block_element ->
    block =
 fun context block ->
  match context with
  | Top_level -> (block :> Odoc_parser_ast.block_element)
...
let rec block_element_list : type block stops_at_which_tokens.
    (block, stops_at_which_tokens) context ->
    parent_markup:[< Odoc_parser_token.t | `Comment ] ->
    input ->
  ...
  let rec consume_block_elements :
   ...
   fun where_in_line acc ->
    let describe token =
      ...
      | _ -> Odoc_parser_token.describe token
    in

    let warn_if_after_text { Odoc_parser_loc.location; value = token } =
      if where_in_line = `After_text then
        Odoc_parser_parse_error.should_begin_on_its_own_line ~what:(describe token) location
        |> add_warning input
    in

    let warn_because_not_at_top_level { Odoc_parser_loc.location; value = token } =
      let suggestion =
        Printf.sprintf "move %s outside of any other markup."
          (Odoc_parser_token.print token)
      in
      Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe token)
        ~in_what:(Odoc_parser_token.describe parent_markup)
        ~suggestion location
      |> add_warning input
    in
    ...
    | { value = `Begin_list_item _ as token; location } ->
        let suggestion =
          Printf.sprintf "move %s into %s, or use %s." (Odoc_parser_token.print token)
            (Odoc_parser_token.describe (`Begin_list `Unordered))
            (Odoc_parser_token.describe `Minus)
        in
        Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          ~suggestion location
    ...
    | { value = `Begin_table_row as token; location } ->
        let suggestion =
          Printf.sprintf "move %s into %s." (Odoc_parser_token.print token)
            (Odoc_parser_token.describe `Begin_table_heavy)
        in
        Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          ~suggestion location
    ...
    | { value = `Begin_table_cell _ as token; location } ->
        let suggestion =
          Printf.sprintf "move %s into %s." (Odoc_parser_token.print token)
            (Odoc_parser_token.describe `Begin_table_row)
        in
        Odoc_parser_parse_error.not_allowed ~what:(Odoc_parser_token.describe token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          ~suggestion location
    ...
    | { value = `Tag tag as token; location } as next_token -> (
        let recover_when_not_at_top_level context =
          ...
          junk input;
          let words = List.map (Odoc_parser_loc.at location) (tag_to_words tag) in
          let paragraph =
            `Paragraph words
            |> accepted_in_all_contexts context
            |> Odoc_parser_loc.at location
          in
        ...
        | Top_level -> (
            if where_in_line <> `At_start_of_line then
              Odoc_parser_parse_error.should_begin_on_its_own_line
                ~what:(Odoc_parser_token.describe token) location
              |> add_warning input;
            ...
            | (`Author s | `Since s | `Version s | `Canonical s) as tag ->
                ...
                if s = "" then
                  Odoc_parser_parse_error.should_not_be_empty ~what:(Odoc_parser_token.describe token)
                    location
                  |> add_warning input;
                let tag =
                  match tag with
                  ...
                  | `Canonical _ ->
                      ...
                      let r_location =
                        Odoc_parser_loc.nudge_start (String.length "@canonical ") location
                      in
                      `Canonical (Odoc_parser_loc.at r_location s)
                in

                let tag = Odoc_parser_loc.at location (`Tag tag) in
            ...
            | ( `Deprecated | `Return | `Children_order | `Short_title
              | `Toc_status | `Order_category ) as tag ->
                ...
                in
                let location =
                  location :: List.map Odoc_parser_loc.location content |> Odoc_parser_loc.span
                in
                let tag = Odoc_parser_loc.at location (`Tag tag) in
            ...
            | `See (kind, target) ->
                ...
                let tag = Odoc_parser_loc.at location tag in
                consume_block_elements where_in_line (tag :: acc)
    ...
    | ( { value = #token_that_always_begins_an_inline_element; _ }
      | { value = `Bar; _ } ) as next_token ->
        ...
        let block = Odoc_parser_loc.map (accepted_in_all_contexts context) block in
        let acc = block :: acc in
        consume_block_elements `After_text acc
    | { value = `Verbatim s as token; location } as next_token ->
        warn_if_after_text next_token;
        if s = "" then
          Odoc_parser_parse_error.should_not_be_empty ~what:(Odoc_parser_token.describe token) location
          |> add_warning input;

        junk input;
        let block = accepted_in_all_contexts context token in
        let block = Odoc_parser_loc.at location block in
        let acc = block :: acc in
        consume_block_elements `After_text acc
    ...
    | {
        ...
        let output, location =
          if not has_outputs then (None, location)
            ...
            junk input;
            let locations =
              location :: List.map (fun content -> content.Odoc_parser_loc.location) content
            in
            let location = Odoc_parser_loc.span locations in
        ...
        let meta =
          match meta with
          | None -> None
          | Some (language, tags) -> Some { Odoc_parser_ast.language; tags }
        in
        let block =
          accepted_in_all_contexts context
            (`Code_block
               {
                 Odoc_parser_ast.meta;
                 delimiter;
    ...
    | { value = `Modules s as token; location } as next_token ->
        ...
        (* TODO Correct locations await a full implementation of {!modules}
           parsing. *)
        let modules =
          split_string " \t\r\n" s |> List.map (fun r -> Odoc_parser_loc.at location r)
        in
    ...
    | { value = (`Minus | `Plus) as token; location } as next_token -> (
        ...
        | _ ->
            ...
            in
            let location =
              location :: List.map Odoc_parser_loc.location (List.flatten items) |> Odoc_parser_loc.span
            in
      ...
      next_token -> (
        ...
        | Top_level ->
            ...
            let label =
              match label with
              | Some "" ->
                  Odoc_parser_parse_error.should_not_be_empty ~what:"heading label" location
                  |> add_warning input;
                  None
              | _ -> label
            in
            ...
            let heading = Odoc_parser_loc.at location heading in
    ...
    | { value = `Begin_paragraph_style _ as token; location } ->
        ...
        Odoc_parser_parse_error.markup_should_not_be_used ~what:(Odoc_parser_token.describe token)
          location
        |> add_warning input;
    ...
    | {
     ...
     value = `Media_with_replacement_text (href, media, content) as token;
        ...
        let r_location =
          Odoc_parser_loc.nudge_start
            (String.length @@ Odoc_parser_token.s_of_media `Replaced media)
            location
          |> Odoc_parser_loc.nudge_end (String.length content + 1)
        ...
        let c_location =
          Odoc_parser_loc.nudge_start
            ...
            location
          |> Odoc_parser_loc.nudge_end 1
        in
        let content = String.trim content in
        let href = href |> Odoc_parser_loc.at r_location in
    ...
    | { location; value = `Simple_media (href, media) } ->
        ...
        let r_location =
          Odoc_parser_loc.nudge_start
            (String.length @@ Odoc_parser_token.s_of_media `Simple media)
            location
          |> Odoc_parser_loc.nudge_end 1
        in
        let href = href |> Odoc_parser_loc.at r_location in
...
and shorthand_list_items :
    ...
    where_in_line ->
    input ->
    Odoc_parser_ast.nestable_block_element with_location list list * where_in_line =
 fun first_token where_in_line input ->
  ...
  let rec consume_list_items :
      [> ] with_location ->
      where_in_line ->
      Odoc_parser_ast.nestable_block_element with_location list list ->
      Odoc_parser_ast.nestable_block_element with_location list list * where_in_line =
   fun next_token where_in_line acc ->
    ...
    | (`Minus | `Plus) as bullet ->
        if bullet = bullet_token then (
          ...
          in
          if content = [] then
            Odoc_parser_parse_error.should_not_be_empty ~what:(Odoc_parser_token.describe bullet)
              next_token.location
            |> add_warning input;
...
and explicit_list_items :
    parent_markup:[< Odoc_parser_token.t ] ->
    input ->
    Odoc_parser_ast.nestable_block_element with_location list list * Odoc_parser_loc.span =
 fun ~parent_markup input ->
  let rec consume_list_items :
   ...
   fun acc ->
    ...
    match next_token.value with
    | `End ->
        Odoc_parser_parse_error.end_not_allowed next_token.location
          ~in_what:(Odoc_parser_token.describe parent_markup)
        |> add_warning input;
        (List.rev acc, next_token.location)
    | `Right_brace ->
    ...
    | `Begin_list_item kind as token ->
           ...
           | `Space _ | `Single_newline _ | `Blank_line _ | `Right_brace ->
               ...
               (* The presence of [`Right_brace] above requires some explanation:
                  ...
                  - The [`Right_brace] token also happens to include all whitespace
                    ...
                    [`Blank_line]. *)
           | _ ->
               Odoc_parser_parse_error.should_be_followed_by_whitespace next_token.location
                 ~what:(Odoc_parser_token.print token)
               |> add_warning input);
        ...
        if content = [] then
          Odoc_parser_parse_error.should_not_be_empty next_token.location
            ~what:(Odoc_parser_token.describe token)
          |> add_warning input;

        (match token_after_list_item.value with
        | `Right_brace -> junk input
        | `End ->
            Odoc_parser_parse_error.end_not_allowed token_after_list_item.location
              ~in_what:(Odoc_parser_token.describe token)
            |> add_warning input);
    ...
    | token ->
        let suggestion =
          ...
          | `Begin_section_heading _ | `Tag _ ->
              Printf.sprintf "move %s outside the list." (Odoc_parser_token.describe token)
          | _ ->
              Printf.sprintf "move %s into a list item, %s or %s."
                (Odoc_parser_token.describe token)
                (Odoc_parser_token.print (`Begin_list_item `Li))
                (Odoc_parser_token.print (`Begin_list_item `Dash))
        in
        Odoc_parser_parse_error.not_allowed next_token.location ~what:(Odoc_parser_token.describe token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
          ~suggestion
        |> add_warning input;
...
and light_table ~parent_markup ~parent_markup_location input =
  let rec consume_rows acc ~last_loc =
    ...
    match next_token.Odoc_parser_loc.value with
    ...
    | other_token ->
        Odoc_parser_parse_error.not_allowed next_token.location
          ~what:(Odoc_parser_token.describe other_token)
          ~in_what:(Odoc_parser_token.describe parent_markup)
        |> add_warning input;
        junk input;
...
and light_table_row ~parent_markup ~last_loc input =
  let rec consume_row acc_row acc_cell acc_space ~new_line ~last_loc =
    ...
    | `Space _ as token ->
        junk input;
        let i = Odoc_parser_loc.at next_token.location token in
    ...
    | #token_that_always_begins_an_inline_element as token ->
        ...
        if Odoc_parser_loc.spans_multiple_lines i then
          Odoc_parser_parse_error.not_allowed
            ~what:(Odoc_parser_token.describe (`Single_newline ""))
            ~in_what:(Odoc_parser_token.describe `Begin_table_light)
            i.location
          |> add_warning input;
        let acc_cell =
...
and heavy_table_row ~parent_markup input =
  let rec consume_cell_items acc =
    ...
    | `Begin_table_cell kind as token ->
        ...
        | `Right_brace -> junk input
        | `End ->
            Odoc_parser_parse_error.not_allowed token_after_list_item.location
              ~what:(Odoc_parser_token.describe `End) ~in_what:(Odoc_parser_token.describe token)
            |> add_warning input);
...
let parse warnings tokens =
  ...
  let rec parse_block_elements () =
    ...
    | `End -> elements
    | `Right_brace ->
        Odoc_parser_parse_error.unpaired_right_brace last_token.location
        |> add_warning input;

        let block =
          Odoc_parser_loc.same last_token (`Paragraph [ Odoc_parser_loc.same last_token (`Word "}") ])
        in
