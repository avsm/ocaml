...
let with_location_adjustments
  ...
  in
  let location = {
    Odoc_parser_loc.file = input.file;
...
let emit =
  with_location_adjustments (fun _ -> Odoc_parser_loc.at)
...
let char_for_decimal_code input lexbuf i =
  ...
  if (c < 0 || c > 255) then
    (warning input (Odoc_parser_parse_error.invalid_char_code c);
    'x')
  else Char.chr c
...
let reference_token media start target input lexbuf =
  ...
  | _ ->
     let target, kind =
       ...
       | _ -> assert false
     in
     let token_descr = Odoc_parser_token.describe (`Media_with_replacement_text (target, kind, "")) in
...
let verbatim_whitespace_first input start_offset text =
  ...
  | _ ->
    warning
      ...
      Odoc_parser_parse_error.no_leading_whitespace_in_verbatim;
    text
...
let heading_level input level =
  if String.length level >= 2 && level.[0] = '0' then begin
    warning
      input ~start_offset:1 (Odoc_parser_parse_error.leading_zero_in_heading_level level)
  end;
  int_of_string level
...
rule reference_paren_content input start ref_offset start_offset depth_paren
  ...
  | eof
    { warning
        input
        ~start_offset
        (Odoc_parser_parse_error.unclosed_bracket ~bracket:"(") ;
      Buffer.contents buffer }
  | _
    {
      buffer_add_lexeme buffer lexbuf ;
...
and reference_content input start start_offset buffer = parse
  ...
  | eof
    { warning
        input
        ~start_offset
        (Odoc_parser_parse_error.unclosed_bracket ~bracket:start) ;
      Buffer.contents buffer }
  | _
    {
      buffer_add_lexeme buffer lexbuf ;
...
and token input = parse
  ...
  | (("{" (delim_char* as delim) "@" horizontal_space*) as prefix) (language_tag_char+ as lang_tag_)
    {
      let start_offset = Lexing.lexeme_start lexbuf in
      let lang_tag =
        with_location_adjustments ~adjust_start_by:prefix (fun _ -> Odoc_parser_loc.at) input lang_tag_
      in
      let emit_truncated_code_block () =
        let empty_content = with_location_adjustments (fun _ -> Odoc_parser_loc.at) input "" in
      ...
      | `Eof ->
          warning input ~start_offset Odoc_parser_parse_error.truncated_code_block_meta;
          emit_truncated_code_block ()
    }

  | "{@" horizontal_space* '['
    {
      warning input Odoc_parser_parse_error.no_language_tag_in_meta;
      code_block false (Lexing.lexeme_start lexbuf) (Lexing.lexeme_end lexbuf) None (Buffer.create 256) "" input lexbuf
    }
      ...
      if e <> "%}" then
        warning
          ...
          (Odoc_parser_parse_error.not_allowed
            ~what:(Odoc_parser_token.describe `End)
  ...
  | '{'
    { try bad_markup_recovery (Lexing.lexeme_start lexbuf) input lexbuf
      with Failure _ ->
        warning
          input
          (Odoc_parser_parse_error.bad_markup
            "{" ~suggestion:"escape the brace with '\\{'.");
        emit input (`Word "{") }

  | ']'
    { warning input Odoc_parser_parse_error.unpaired_right_bracket;
      emit input (`Word "]") }

  | "@param"
    { warning input Odoc_parser_parse_error.truncated_param;
      emit input (`Tag (`Param "")) }

  | ("@raise" | "@raises") as tag
    { warning input (Odoc_parser_parse_error.truncated_raise tag);
      emit input (`Tag (`Raise "")) }

  | "@before"
    { warning input Odoc_parser_parse_error.truncated_before;
      emit input (`Tag (`Before "")) }

  | "@see"
    { warning input Odoc_parser_parse_error.truncated_see;
      emit input (`Word "@see") }

  | '@' ['a'-'z' 'A'-'Z']+ as tag
    { warning input (Odoc_parser_parse_error.unknown_tag tag);
      emit input (`Word tag) }

  | '@'
    { warning input Odoc_parser_parse_error.stray_at;
      emit input (`Word "@") }

  | '\r'
    { warning input Odoc_parser_parse_error.stray_cr;
      token input lexbuf }
  ...
  | "{!modules:" ([^ '}']* as modules) eof
    { warning
        ...
        (Odoc_parser_parse_error.not_allowed
          ~what:(Odoc_parser_token.describe `End)
          ~in_what:(Odoc_parser_token.describe (`Modules "")));
      emit input (`Modules modules) }
...
and code_span buffer nesting_level start_offset input = parse
  ...
  | newline horizontal_space* (newline horizontal_space*)+
    { warning
        input
        (Odoc_parser_parse_error.not_allowed
          ~what:(Odoc_parser_token.describe (`Blank_line "\n\n"))
          ~in_what:(Odoc_parser_token.describe (`Code_span "")));
      Buffer.add_char buffer ' ';
      code_span buffer nesting_level start_offset input lexbuf }
  ...
  | (newline) as s
    {
      ...
      | Inline ->
        warning
          ...
          (Odoc_parser_parse_error.not_allowed
            ~what:(Odoc_parser_token.describe (`Blank_line "\n"))
            ~in_what:(Odoc_parser_token.describe (math_constr kind "")));
        Buffer.add_char buffer '\n';
        math kind buffer nesting_level start_offset input lexbuf
      | Block ->
        Buffer.add_string buffer s;
    ...
    { begin match last_false_terminator with
      | None ->
        warning
          ...
          (Odoc_parser_parse_error.not_allowed
            ~what:(Odoc_parser_token.describe `End)
            ~in_what:(Odoc_parser_token.describe (`Verbatim "")))
      | Some location ->
        warning
          input
          ~start_offset:location
          ~end_offset:(location + 2)
          Odoc_parser_parse_error.no_trailing_whitespace_in_verbatim
      end;
      emit_verbatim input start_offset buffer }
  ...
  | [^ '}']+ as text '}' as rest
    { let suggestion =
      ...
      warning
        input
        ~start_offset
        (Odoc_parser_parse_error.bad_markup ("{" ^ rest) ~suggestion);
 ...
 | '\\' (['\\' '\'' '\"' 'n' 't' 'b' 'r' ' '] as c)
  ...
  | '\\' ['0'-'9'] ['0'-'9'] ['0'-'9']
    { Buffer.add_char input.string_buffer (char_for_decimal_code input lexbuf 1);
      string input lexbuf }
  | '\\' (_ as c)
    { warning input (Odoc_parser_parse_error.should_not_be_escaped c);
      Buffer.add_char input.string_buffer c;
      string input lexbuf }
  | eof
    { warning input Odoc_parser_parse_error.truncated_string;
 ...
 | '"'
   {
    ...
    with_location_adjustments ~start_offset (fun _ -> Odoc_parser_loc.at) input s }
 | (tag_unquoted_atom as value)
   { with_location_adjustments (fun _ -> Odoc_parser_loc.at) input value }
 | ('=' as c) 
   { warning input (Odoc_parser_parse_error.code_block_tag_invalid_char c);
     with_location_adjustments (fun _ -> Odoc_parser_loc.at) input "" }
...
and code_block_metadata_tail input tag acc = parse
 ...
 | '='
   { match tag with
     ...
     | None ->
       warning input (Odoc_parser_parse_error.code_block_tag_invalid_char '=');
       code_block_metadata_tail input None acc lexbuf }
 | (_ # space_char # '[' # '=' as c) (_ # space_char # '[')*
   {
      let start_offset = Lexing.lexeme_start input.lexbuf in
      let end_offset = start_offset + 1 in
      warning input ~start_offset ~end_offset (Odoc_parser_parse_error.code_block_tag_invalid_char c);
    code_block_metadata_tail input None acc lexbuf }
 | eof
    { `Eof }
...
and code_block allow_result_block start_offset content_offset metadata prefix delim input = parse
  ...
  | eof
    {
      warning input ~start_offset Odoc_parser_parse_error.truncated_code_block;
